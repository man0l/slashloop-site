...
export default function ExperimentCreate({ accessToken, workspaceId, videoIds, onClose }) {
  ...
  return <section className="my-6 rounded-xl p-5 sm:p-6" style={{ background: T.card, border: `1px solid ${T.line}` }} aria-label="Create experiment">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 style={{ ...fD, fontSize: 24, fontWeight: 800 }}>Create an experiment</h2><p className="mt-1 text-sm" style={{ color: T.muted }}>{videoIds.length} originals selected · Free draft</p></div><ExperimentButton onClick={onClose} disabled={busy}>Close setup</ExperimentButton></div>
    <form onSubmit={submit} className="mt-5 space-y-5">
      ...
      <p className="text-xs" style={{ color: T.muted }}>Total spending cap, not a quote. Each paid stage needs approval.</p>
