...
export default function ExperimentVariant({ variant, baseline, expectedSlideCount, index, selected, selectable, busy, onSelect, onSave, onDirty, onDownload, onSchedule, retryJobFor, onRetrySlide }) {
  ...
  return <article className="rounded-xl p-5 space-y-4 min-w-0" style={{ background: T.card, border: `1px solid ${selected ? T.teal : T.line}` }}>
    ...
    {slides.length > 0 && <section aria-label={`${variant.title || "Variant"} images`}>
      ...
      <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))" }}>{slides.map((slide, i) => <figure key={slide.index ?? i} className="min-w-0"><div className="rounded-lg flex items-center justify-center overflow-hidden" style={{ background: T.paper, aspectRatio: "9/16", border: `1px solid ${T.line}` }}>{slide.url ? <a href={slide.url} target="_blank" rel="noreferrer" className="w-full h-full" aria-label={`Open slide ${i + 1} full size`}><img src={slide.url} alt={slide.overlayText || `Generated slide ${i + 1}`} loading="lazy" className="w-full h-full object-contain" /></a> : <span className="text-xs p-3 text-center" style={{ color: T.muted }}>{slide.status || "Waiting"}</span>}</div><figcaption className="text-xs mt-2" style={{ color: slide.error ? "#9B2C23" : T.muted }}>Slide {i + 1} · {slide.status}{slide.error ? ` · ${errorText(slide.error)}` : ""}</figcaption>{onRetrySlide && retryJobFor?.(variant.id, slide.index) && <button type="button" disabled={busy} onClick={() => onRetrySlide(retryJobFor(variant.id, slide.index))} className="mt-2 rounded-md border px-2 py-1 text-xs font-semibold hover:opacity-80 disabled:opacity-50" style={{ borderColor: T.line, color: "#9B2C23", background: T.card }}>Retry slide {i + 1}</button>}</figure>)}</div>
