...
import { SectionLabel, Spinner, ConfirmDialog } from "../components/ui.jsx";
...
function Status({ status }) {
  const s = status?.replaceAll("_", " ") || "draft";
...
function Pipeline({ experiment, completedInputs }) {
  ...
  const imageCount = slides.filter((s) => s.url && ["done", "completed"].includes(s.status)).length;
  ...
  const imageFailure = !completed && (failedImages > 0 || generated.some((v) => ["failed", "partial"].includes(v.status) && !hasUnknownOutcome(v)));
...
export function ExperimentDetail({ accessToken, workspaceId, experimentId }) {
  ...
  // Briefs start selected — the user reviews and unchecks instead of ticking every box.
  useEffect(() => {
    if (autoSelected.current || !experiment || !variants.length) return;
    autoSelected.current = true;
    setSelected(variants.filter((v) => canSelect(v)).map((v) => v.id));
  }, [experiment, variants]);
  // Bring the credit approval into view the moment it appears.
  useEffect(() => { approvalRef.current?.scrollIntoView?.({ behavior: "smooth", block: "center" }); }, [approval]);
  ...
  const allDone = experiment?.status === "completed" || (variants.length > 0 && variants.every((v) => ["done", "completed"].includes(v.status)));
  const unknown = !allDone && (experiment?.status === "paused" || hasUnknownOutcome(experiment?.error) || experiment?.jobs?.some((j) => j.status === "unknown") || variants.some((v) => hasUnknownOutcome(v.error) || v.slides?.some((s) => hasUnknownOutcome(s.error))) || experiment?.inputs?.some((i) => hasUnknownOutcome(i.error)));
  const knownFailed = variants.filter((v) => !hasUnknownOutcome(v) && (v.status === "failed" || v.status === "partial" || v.slides?.some((s) => s.status === "failed")));
  // Mirrors backend MAX_MANUAL_ATTEMPTS: after 6 total attempts a job is no longer retryable.
  ...
  async function prepare(action, stage, payload = {}) {
    // Unknown outcomes block blanket spending, but a named single-job retry is an explicit choice.
    if (lock.current || active || anyDirty || (unknown && !payload.taskIds?.length)) return;
    ...
    try {
      ...
      const estimate = await estimateExperiment(accessToken, workspaceId, experimentId, stage, ids, estimateAbort.current.signal, payload.taskIds);
  ...
  async function removeExperiment() {
    setDeleting(true);
    try { await deleteExperiment(accessToken, workspaceId, experimentId); await qc.invalidateQueries({ queryKey: experimentKey(accessToken, workspaceId) }); navigate("/experiments"); }
    catch (err) { if (mounted.current) setProblem(err.message || "Delete failed."); }
    finally { if (mounted.current) { setDeleting(false); setConfirmDelete(false); } }
  }
  ...
  return <div className="space-y-6">
    ...
    <details open={!hasOutputs} className="rounded-xl p-5" style={panel}><summary className="cursor-pointer font-semibold">Source analysis & pattern report</summary><section className="mt-4"><h2 style={{ ...fD, fontWeight: 800, fontSize: 20 }}>Source analysis <span className="text-sm font-normal" style={{ color: T.muted }}>{completedInputs}/{experiment.inputs?.length ?? 0} ready</span></h2><div className="mt-3 space-y-2">{experiment.inputs?.map((input) => { const job = retryableJob("analysis", input.videoId); return <div key={input.videoId} className="flex flex-wrap items-center justify-between gap-2 text-sm rounded-lg p-3" style={{ background: T.paper }}><Link to={`/gallery?video=${encodeURIComponent(input.videoId)}`} className="underline break-all">Original {(experiment.inputs?.indexOf(input) ?? 0) + 1}</Link><span className="flex flex-wrap items-center gap-2"><span>{input.status}{input.error ? ` · ${errorText(input.error)}` : ""}{input.coverage ? ` · Coverage: ${displayValue(input.coverage)}` : ""}</span>{job && <ExperimentButton disabled={busy} onClick={() => retryJob(job)}>Retry analysis</ExperimentButton>}</span></div>; })}</div></section>
