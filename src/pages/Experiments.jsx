...
export default function Experiments() {
  ...
  return <main className="max-w-6xl mx-auto px-5 py-12 min-h-[70vh]">
    ...
    <div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 style={{ ...fD, fontSize: 32, fontWeight: 900, letterSpacing: -0.8 }}>From evidence to variants</h1><p className="mt-2 text-sm" style={{ color: T.muted }}>Analyze originals. Test one change. Review before you generate.</p></div><Link to="/gallery" className="rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: T.signal, color: "white" }}>Select Gallery originals</Link></div>
...
export function ExperimentDetail({ accessToken, workspaceId, experimentId }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl p-5 sm:p-6 space-y-4" style={panel}>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm" style={{ color: T.muted }}><span>{experiment.instructions?.mode === "exploration" ? "Exploration" : "Controlled · one variable per variant"}</span><span>{experiment.variantCount} variants · {experiment.slideCount} slides each</span><span>{experiment.creditsCharged ?? 0} credits charged · {experiment.maxCredits} credit ceiling</span></div>
      <details><summary className="text-sm cursor-pointer">Instructions & constraints</summary><dl className="mt-3 grid sm:grid-cols-2 gap-3 text-sm">{Object.entries(experiment.instructions ?? {}).map(([key, value]) => <div key={key}><dt className="font-semibold">{key}</dt><dd className="whitespace-pre-wrap break-words" style={{ color: T.muted }}>{Array.isArray(value) ? value.join(", ") : displayValue(value)}</dd></div>)}</dl></details>
      <p className="text-xs" style={{ color: T.muted }}>Sources → analysis → pattern report → manual brief review → selected images. Text-directed synthesis, not exact source copies. Nothing is published automatically.</p>
      ...
      {active && <p role="status" className="text-sm" style={{ color: T.teal }}>Working on the server. You can leave and return; this page refreshes while work is active. Cancellation stops queued steps, not work already sent to a provider.</p>}
    ...
    {unknown && <p role="alert" className="rounded-lg p-4 text-sm" style={panel}>A provider outcome is unknown. Paid actions and retries are paused to prevent duplicate charges. Refresh status or contact support; do not start a replacement request.</p>}
    {uncertain && !unknown && <div className="rounded-lg p-4 text-sm space-y-3" style={panel}><p>The request response was lost. Refresh status first. Rechecking below resends only the identical request key, never a new generation.</p><ExperimentButton disabled={busy} onClick={() => dispatch(uncertain)}>Recheck original request</ExperimentButton></div>}
    <section className="rounded-xl p-5" style={panel}><h2 style={{ ...fD, fontWeight: 800, fontSize: 20 }}>Source analysis <span className="text-sm font-normal" style={{ color: T.muted }}>{completedInputs}/{experiment.inputs?.length ?? 0} ready</span></h2><div className="mt-3 space-y-2">{experiment.inputs?.map((input) => <div key={input.videoId} className="flex flex-wrap justify-between gap-2 text-sm rounded-lg p-3" style={{ background: T.paper }}><Link to={`/gallery?video=${encodeURIComponent(input.videoId)}`} className="underline break-all">Original {input.videoId}</Link><span>{input.status}{input.error ? ` · ${errorText(input.error)}` : ""}{input.coverage ? ` · Coverage: ${displayValue(input.coverage)}` : ""}</span></div>)}</div></section>
